...
def classifierCNN(input_shape, name='cls_cnn', load_weights=None):
    ...
    model.add(Lambda(lambda x: (x-127)/127, input_shape=input_shape))  # normalize 0 mean
    model.add(Conv2D(filters=9, kernel_size=(3, 3), activation='relu', name='conv_0', padding="same"))
    model.add(Dropout(0.6))
    model.add(Conv2D(filters=18, kernel_size=(3, 3), activation='relu', name='conv_1', padding="same"))
    model.add(Dropout(0.6))
    model.add(Conv2D(filters=27, kernel_size=(3, 3), activation='relu', name='conv_2', padding="same"))
    model.add(Dropout(0.6))
    model.add(Conv2D(filters=36, kernel_size=(3, 3), activation='relu', name='conv_3', padding="same"))
    model.add(Dropout(0.6))
    model.add(Conv2D(filters=54, kernel_size=(3, 3), activation='relu', name='conv_4', padding="same"))
    model.add(MaxPool2D(pool_size=(8, 8)))
    # binary 'classifier'
    model.add(Conv2D(filters=1, kernel_size=(8, 8), activation="tanh", name='classifier'))


    if load_weights is not None and os.path.isfile(load_weights):
...
def train(dataset, epochs=30, batch_size=32, load_weights=None, debug=False):
    ...
    earlystopping = EarlyStopping(monitor='val_acc', min_delta=0.001, patience=3, verbose=1)
